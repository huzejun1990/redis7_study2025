...
//        Integer[] splitRedPackages = null; //拆分红包算法通过后获得的多个子红包数组
...
//        Object redPackage = redisTemplate.opsForHash().get(RED_PACKAGE_KEY + redPackageKey, userId);
        ...
        if (null == redPackage)
        {
            ...
            {
                //2.2 抢到红包后需要记录进入hash结构，表示谁抢到了多少钱的某个红包
    ...
    private Integer[] splitRedPackageAlgorithm(int totalMoney, int redPackageNumber) {
        ...
        for (int i = 0; i < redPackageNumber; i++) {
            if (i == redPackageNumber - 1) {
                ...
                // 二倍均值算法，每次拆分后放进子红包的金额  = 随机区间（0，（剩余红包金额 M / 未被抢的剩余红包个数N）* 2）
